...
xdebug_start_trace();
 ...
 if($_POST["addterm"] == 1 && $_POST["startdate"] != "" && $_POST["title"] != "" && $_POST["enddate"] != "")
 {
  ...
  $query = mysql_query("INSERT INTO terms (title, startdate, enddate) VALUES( '$_POST[title]', '$_POST[startdate]', '$_POST[enddate]')")
 ...
 if($_POST["editterm"] == 1 && $_POST["startdate"] != "" && $_POST["title"] != "" && $_POST["enddate"] != "")
 {
  ...
  $startdate = converttodb($_POST["startdate"]);
  $enddate = converttodb($_POST["enddate"]);
  $query = mysql_query("UPDATE `terms` SET `title`='zio', `startdate`='$startdate', `enddate`='$enddate' WHERE `termid`='$termid' LIMIT 1")
